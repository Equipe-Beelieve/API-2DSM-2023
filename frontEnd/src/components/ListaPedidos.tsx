    ...
    return(
        ...
        <div className="mainContent">
            <div className="state">
                ...
                <button className={`a_caminho ${filtroStatus === 'A caminho' ? 'active' : ''}`}
                onClick={(evento) => setFiltroStatus(filtroStatus === 'A caminho' ? '' : 'A caminho')}>A caminho</button>
                <button className={`em_analise ${filtroStatus === 'Em análise' ? 'active': ''}`} 
                onClick = {(evento) => setFiltroStatus(filtroStatus === 'Em análise' ? '' : 'Em análise')}>Em análise</button>
                <button className={`finalizado ${filtroStatus === 'Finalizado' ? 'active' : ''}`} onClick = {(evento) => setFiltroStatus(filtroStatus === 'Finalizado' ? '' : 'Finalizado')}>Finalizado</button>
